...
fit_frechet_regression <- function(x_new, data_curves, x, knots = seq(0,1,0.2), type = "smooth",
                                   closed = FALSE, eps = 0.01, max_iter = 10){
  # input checking for closed curves
  if(closed) data_curves <- lapply(data_curves, elasdics:::check_closed)
  ...
  coefs_list <- lapply(x_new, function(x_new){
    ...
    while(stop_crit > eps & k < max_iter){
      ...
      if(type == "polygon"){
        srv_data_discrete <- sapply(1:length(data_curves), function(i){
          curve_data <- get_evals(cbind("t" = t_optims[[i]], data_curves[[i]][, -(1:2)]), t_grid = knots)
          unlist(get_srv_from_points(cbind("t" = knots, curve_data))[, -1]*weights[i])
        })
        coefs <- apply(srv_data_discrete, 1 ,mean, na.rm = TRUE)
        coefs <- matrix(coefs, ncol = ncol(data_curves[[1]]) - 2)
      } else {
